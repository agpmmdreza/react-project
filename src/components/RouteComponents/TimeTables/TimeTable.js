...
const TimeTable = () => {
  ...
  const handleGetTablesById = () => {
    // Code For Getting Time Tables By ID
    if (validation.value.trim() !== '') {
      fetch(`${authCtx.baseURL}/api/timetables/${validation.value}`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${authCtx.token}`,
        },
        ...
        <div className={classes.header}>
          ...
          <div>
            ...
            <Button
              variant='contained'
              color='primary'
              onClick={handleGetTablesById}
            >
              Get Time Tables By id
            </Button>
          </div>
        </div>
                      ...
                      .map((row) => (
                        <TableRow key={row.id} hover>
                          ...
                          <TableCell align='left'>
                            {row.master.user.firstName +
                              ' ' +
                              row.master.user.lastName}
                          </TableCell>
                          <TableCell>
                            {row.timeTableBellList.map((item) => (
                              <p>
                          ...
                          <TableCell align='left'>{row.course.title}</TableCell>
                        </TableRow>
                      ))}
                  </TableBody>
                </Table>
